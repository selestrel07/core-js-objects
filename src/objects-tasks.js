...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    const item = queue[i];
    if (item > 25 + exchange) return false;
    exchange += 50 - item;
  }
  return true;
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((element) => {
    const value = createdMap.get(keySelector(element)) || [];
    value.push(valueSelector(element));
    createdMap.set(keySelector(element), value);
  });
  return createdMap;
}
...
const cssSelectorBuilder = {
  ...
  element(value) {
    ...
    const type = 'element';
    if (instance.structure.includes(type)) {
      this.throwUniquenessError();
    }
    if (
      instance.structure.some((el) =>
        ['pseudo-element', 'pseudo-class', 'attr', 'class', 'id'].includes(el)
      )
    ) {
      this.throwOrderError();
    }
    instance.structure.push(type);
  ...
  id(value) {
    ...
    if (
      instance.structure.some((el) =>
        ['pseudo-element', 'pseudo-class', 'attr', 'class'].includes(el)
      )
    ) {
      this.throwOrderError();
    }
  ...
  class(value) {
    ...
    if (
      instance.structure.some((el) =>
        ['pseudo-element', 'pseudo-class', 'attr'].includes(el)
      )
    ) {
      this.throwOrderError();
    }
    instance.structure.push('class');
    instance.value += `.${value}`;
    return instance;
  },
  ...
  attr(value) {
    ...
    if (
      instance.structure.some((el) =>
        ['pseudo-element', 'pseudo-class'].includes(el)
      )
    ) {
      this.throwOrderError();
    }
    instance.structure.push('attr');
    instance.value += `[${value}]`;
    return instance;
  },
  ...
  pseudoClass(value) {
    const instance = this.getInstance();
    if (instance.structure.includes('pseudo-element')) {
      this.throwOrderError();
    }
    instance.structure.push('pseudo-class');
    instance.value += `:${value}`;
    return instance;
  },
  ...
  combine(selector1, combinator, selector2) {
    const instance = this.getInstance();
    instance.value = `${selector1.stringify()} ${combinator} ${selector2.stringify()}`;
    return instance;
  },
